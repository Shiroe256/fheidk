...
class BillingController extends Controller
{
    ...
    SUM(
        (
            CASE
                ...
                WHEN (
                    (
                        (`tbl_other_school_fees`.`type_of_fee` = 'school id' AND `tbl_other_school_fees`.`coverage` ='per new student') OR (`tbl_other_school_fees`.`type_of_fee` = 'school id' AND `tbl_other_school_fees`.`coverage` = 'per student')
                    )
                    AND (
                        `students_sub`.`transferee` = 'yes'
                    )
    ...
    function generateForm2($pdf_data, $signatories,  $grantees)
    {
        ...
        foreach ($grantees as $key => $grantee) {
            ...
            $granteeRow =     array(
                ...
                'total_tosf' => number_format($grantee->total_fee, 2)
    ...
    public function fetchTempSummary(Request $request)
    {
        ...
        $data['total_fee'] = (int)$data['form2_total'] + (int)$data['form3_total'];
...
sum(if(tbl_other_school_fees.category = "Computer Laboratory", tbl_other_school_fees.amount * students_sub.comp_lab_unit, 0)) as total_fees')
    ...
    public function billingmanagementpage_Jojo($reference_no)
    {
        $billings = Billing::where('reference_no', $reference_no)->first();
        $hei_uii = Auth::user()->hei_uii;
        ...
        $data2['hei_psg_region'] = $billings->hei_psg_region;
        $data2['ac_year'] = $billings->ac_year;
        $data2['semester'] = $billings->semester;
        $data2['tranche'] = $billings->tranche;
        $data2['reference_no'] = $billings->reference_no;
        $data2['billing_status'] = $billings->billing_status;
        return view('billingmanagement-attachments', $data2);
    }
