...
class computeStudentFees implements ShouldQueue
{
    ...
    public function __construct($reference_no, $uids)
    {
    ...
    public function handle()
    {
        ...
        $students_sub = DB::table('tbl_billing_details_temp')->where('tbl_billing_details_temp.reference_no', '=', $reference_no)->whereIn('uid', $uids);
...
sum(if(tbl_other_school_fees.category = "Computer Laboratory", tbl_other_school_fees.amount * students_sub.comp_lab_unit, 0)) as total_fees')
            ...
            ->groupBy('students_sub.uid')->get()->toArray();
        ...
        if (count($students) > 0) {
            ...
            $caseStatements = collect($students)->map(function ($row) use ($idColumn) {
                return "WHEN {$row[$idColumn]} THEN '{$row['total_fees']}'";
            })->implode(' ');

            $idValues = implode(',', array_column($students, $idColumn));
