...
//EDIT AFC
// edit fee ajax request
$(document).on('click', '.btn_link_afc', function (e) {
    e.preventDefault();
    let reference_no = $(this).attr('id');
    // alert(reference_no);
    $.ajax({
      ...
      success: function (response) {
        // alert();
        // console.log(response);
        $('#link_afc').val(response.afc_link);
      }
    });
  });
  
  $("#frm_link_afc").submit(function (e) {
    e.preventDefault();
    const fd = new FormData(this);
    $("#btn_attach_afc").text('Attaching');
    $.ajax({
        url: '/updatelinkafc',
        method: 'post',
        data: fd,
        ...
        success: function (response) {
            if (response.status == 200) {
                ...
                .then(() => {
                    $("#btn_attach_afc").text('Attach');
                    $("#frm_link_afc")[0].reset();
                    $("#mod_upload_link_afc").modal('hide');
                    // Reload the specific div
                    $("#tbl_billing_attachments_div").load(location.href + " #tbl_billing_attachments_div");
                });
            } else if (response.status == 400) {
